...
DESTINATION=${PREFIX}/share/jokeralpine
choice=""
...
addresolvconf ()
{
  ...
  [ \$(command -v getprop) ] && getprop | sed -n -e 's/^\[net\.dns.\]: \[\(.*\)\]/\1/p' | sed '/^\s*$/d' | sed 's/^/nameserver /' > \${PREFIX}/share/jokeralpine/etc/resolv.conf
  fi
}
addresolvconf
exec proot --link2symlink -0 -r \${PREFIX}/share/jokeralpine/ -b /dev/ -b /sys/ -b /proc/ -b /sdcard -b /storage -b \$HOME -w /home /usr/bin/env HOME=/root PREFIX=/usr SHELL=/bin/sh TERM="\$TERM" LANG=\$LANG PATH=/bin:/usr/bin:/sbin:/usr/sbin /bin/sh --login
EOM
...
finalwork() {
	[ ! -e ${DESTINATION}/joker.sh ] && curl --silent -LO https://raw.githubusercontent.com/joker-termux/joker-termux-alpine/master/joker.sh
	if [ "${MOTD}" = "ON" ]; then
		bash ${DESTINATION}/joker.sh --add-motd
	else
		bash ${DESTINATION}/joker.sh
	fi
	rm ${DESTINATION}/joker.sh
}
...
cleanup() {
	...
	if [ -e ${PREFIX}/bin/jokeralpine ]; then
		rm ${PREFIX}/bin/jokeralpine
...
printf "$blue [*] GitHub :${yellow}    https://github.com/joker-termux\n"
printline
